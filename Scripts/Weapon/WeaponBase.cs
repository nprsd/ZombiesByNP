...
public class WeaponBase : MonoBehaviour
{
    ...
    void CreateWater(Vector3 pos, Vector3 normal)
    {		
		GameObject water = prefabManager.GetPrefab("WaterEffect");
		GameObject waterEffect = Instantiate(water, pos, Quaternion.LookRotation(normal));
		Destroy(waterEffect, 1.5f);
	}
    void CreateSand(Vector3 pos, Vector3 normal)
    {
        GameObject sand = prefabManager.GetPrefab("SandEffect");
        GameObject sandEffect = Instantiate(sand, pos, Quaternion.LookRotation(normal));
        Destroy(sandEffect, 1.5f);
    }
    void CreateMetal(Vector3 pos, Vector3 normal)
    {
        GameObject metal = prefabManager.GetPrefab("MetalEffect");
        GameObject metalEffect = Instantiate(metal, pos, Quaternion.LookRotation(normal));
        Destroy(metalEffect, 1.5f);
    }
    void CreateWood(Vector3 pos, Vector3 normal)
    {
        GameObject wood = prefabManager.GetPrefab("WoodEffect");
        GameObject woodEffect = Instantiate(wood, pos, Quaternion.LookRotation(normal));
        Destroy(woodEffect, 1.5f);
    }
    void CreateStone(Vector3 pos, Vector3 normal)
    {
        GameObject stone = prefabManager.GetPrefab("StoneEffect");
        GameObject stoneEffect = Instantiate(stone, pos, Quaternion.LookRotation(normal));
        Destroy(stoneEffect, 1.5f);
    }
